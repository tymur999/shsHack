...
export default class CreateItem extends Component<ItemProps, ImageData> {
    ...
    async createItem(cost: number, name: string, desc: string, imgurl: string){
        await firestore.collection("Rentals").doc(name).set({
            cost: cost,
    ...
    async componentDidMount() {
        ...
        const cost = (Number)((document.querySelector("input[name='cost']") as HTMLInputElement).value);
        await this.createItem(cost, name, desc, imgurl);
    }
